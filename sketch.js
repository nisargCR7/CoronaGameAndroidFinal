...
var Seconds=[]
var Name=[]
var qstate=0,cstate=0,wstate=0,rstate=0,musicstate=1,mstate=0;
var r,rr=7
var Options=[]
var Answers=[]
var ans=0
...
function setup() {
 ...
 wall48 = createSprite(Width/2.05,Height/1.68,Width/5.2,Width/95)
 ...
 mazerunner = createSprite(Width/1.145,Height/8,Width/25,Height/20)
...
function touchEnded(){
  ...
  function keyPressed(){
    ...
    if(keyCode===13){
    if(gameState===9){
      player_ans=game.inputbox.value()
      ...
      game.inputbox.value("")
      game.txt.hide()
      if(player_ans===Words[game.rand2]){
        wstate="right"
      ...
      Scrambled.splice(game.rand2,1)
      Words.splice(game.rand2,1)
      Questions2.splice(game.rand2,1)
      game.submit.hide()  
      game.inputbox.hide()
      enterstate=0
    ...
    if(gameState===11){
      ...
      game.inputbox.value("")
      if(player_ans===ansOfRiddle[game.rand4]){
        rstate="right"
        }else{
          rstate="wrong"
        }
        ansOfRiddle.splice(game.rand4,1)
        Riddle.splice(game.rand4,1)
        Hint.splice(game.rand4,1)
        enterstate=0
    ...
    if(gameState===2){
      ...
      if(player.name===""){
        form.txt.show() 
        form.txt.html("You can not leave your name blank")
        form.txt.size()
        form.txt.style('left',Width/2-Width/10+'px')
        form.txt.style('right',Width/20+'px')
        form.txt.style('fontSize',Width/40+"px")
        form.txt.style('fontWeight','bold')
        form.txt.style('fontFamily','arial')
        form.txt.style('color','red')
        form.txt.position(Width/2-Width/5,Height/10*9);
      }else{
       form.input.hide();
       form.button.hide();
      crowd_music.play()
